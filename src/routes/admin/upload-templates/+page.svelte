...
async function addTemplate() {
    try {
        if (!file || !base64Image || !title || !category) {
...
async function fetchImages() {
    try {
        ...
        images = querySnapshot.docs.map(doc => ({ ...doc.data(), templateId: doc.id })); // Ensure unique key
    } catch (error) {
...
<main>
  ...
  <!-- Upload Modal -->
  {#if formModalOpen}
      <Modal on:close={() => formModalOpen = false}>
          <div class="p-6">
              ...
              <Input id="category" bind:value={category} class="mb-4" placeholder="Enter category" />
