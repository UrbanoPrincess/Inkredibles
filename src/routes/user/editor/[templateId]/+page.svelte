<script lang="ts">
    ...
    let textColor: string = "#000000"; // Default color: Black
    let fontFamily: string = "Great Vibes"; // Default font
    let fontSize: string = "text-lg"; // Default size (medium)
  
    function updateContent(event: Event, type: "image" | "text") {
      const target = event.target as HTMLInputElement;
  
      if (!target) return; // Ensuring target is not null
